...
public class DropDownViewDataSource: NSObject, UITableViewDataSource {
    ...
    var presenter: DropDownView!
    
    required public init(option: [Any] = [Any](), presenter: DropDownView) {
        self.options = option
    ...
    private func setupDropDownViewCell(indexPath: Int) ->  UITableViewCell {
        ...
        case .image:
            ...
            imageView.center = CGPoint(x: presenter.tableView.frame.width / 2, y: cell.frame.height / 2)
        ...
        case .string:
            let label = UILabel(frame: CGRect(x: 5, y: 0, width: presenter.tableView.frame.width  - 10, height: cell.frame.height))
